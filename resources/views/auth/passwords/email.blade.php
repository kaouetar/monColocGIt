...
animation-name: fadeInUp;
  ...
  <div class="container">
  	<div class="login-container">
              <div id="output"></div>
              <div class="avatar"> <span class="glyphicon glyphicon-lock" aria-hidden="true"></span> </div>
              <div class="form-box">
                <form action="{{ route('password.email') }}" method="POST">
                      @csrf
                    <div class="card-header"><h4>{{ __('Réinitialisation du mot de passe') }}</h4></div>

                    <input id="email" type="email" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ $email ?? old('email') }}" placeholder="Adresse e-mail" required autofocus>
